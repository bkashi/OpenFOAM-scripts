...
# Hotkeys:
#   [q] Quit Monitor | [K] Kill foamRun | [1]-[5] Adjust refresh rate (1s to 30s)
# ==============================================================================
...
probe_list = system("ls -1 postProcessing/ | grep -v 'residuals'")
...
if (system("test -e postProcessing/residuals/0/residuals.dat && echo 1 || echo 0") == 1) {
    while (1) { 
        ...
        last_save = system("foamListTimes -latestTime -processor 2>/dev/null")
        ...
        # Initialize plot command with residuals
        plot_string = "'< tail -n '.n_iter.' postProcessing/residuals/'.latest_res.'/residuals.dat' skip s u 1:2 axes x1y1 w l lw 2 dt 2 title 'Ux Res', "
        plot_string = plot_string . "'' skip s u 1:3 axes x1y1 w l lw 2 dt 2 title 'Uy Res', "
        plot_string = plot_string . "'' skip s u 1:5 axes x1y1 w l lw 2 dt 2 title 'p Res', "
        plot_string = plot_string . "'' skip s u 1:6 axes x1y1 w l lw 2 dt 2 title 'T Res'"

        # Loop through all probes to gather stats and add to plot string
        do for [probe in probe_list] {
            latest_time = system("ls -v postProcessing/".probe."/ | tail -n 1")
            ...
            if (system("test -e ".path."p && echo 1 || echo 0") == 1) {
                ...
                last_val = system("tail -n 1 ".path."p | awk '{print $2}'")
                prev_val = system("tail -n ".c_iter." ".path."p | head -n 1 | awk '{print $2}'")
            ...
            if (system("test -e ".path."U && echo 1 || echo 0") == 1) {
                ...
                plot_string = plot_string . ", '< tail -n '.n_iter.' ".path."U' skip s u 1:((($2**2+$3**2+$4**2)**0.5)/U_".probe."_max) \"%lf (%lf %lf %lf)\" axes x1y2 w l lw 3 title 'U ".probe."'"
            }
            
            # Process Temperature (T)
            if (system("test -e ".path."T && echo 1 || echo 0") == 1) {
                stats '< tail -n '.n_iter.' '.path.'T' skip s u 2 name 'T_'.probe nooutput
                last_val = system("tail -n 1 ".path."T | awk '{print $2}'")
                prev_val = system("tail -n ".c_iter." ".path."T | head -n 1 | awk '{print $2}'")
                conv = (real(prev_val) != 0) ? sprintf("%.1f", (last_val - prev_val)/prev_val * 100) : "..."
